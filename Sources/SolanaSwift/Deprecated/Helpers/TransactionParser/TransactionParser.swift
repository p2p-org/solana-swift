    ...
    private func calculateFee(transactionInfo: TransactionInfo, feePayerPubkeys: [String]) -> Single<FeeAmount> {
        ...
        return Single.zip(
            ...
            .map { [weak self] lamportsPerSignature, minRentExemption in
                ...
                let createWSOLAccountInstructions = confirmedTransaction.message.instructions.filter {$0.programId == PublicKey.programId.base58EncodedString && $0.parsed?.type == "createAccount"}
                let closeAccountInstructions = confirmedTransaction.message.instructions.filter {$0.programId == PublicKey.tokenProgramId.base58EncodedString && $0.parsed?.type == "closeAccount"}
