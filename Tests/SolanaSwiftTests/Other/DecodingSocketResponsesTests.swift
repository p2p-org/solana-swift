...
class DecodingSocketResponsesTests: XCTestCase {
    ...
    func testDecodingSOLAccountNotification() throws {
        ...
        let result = try decoder.decode(Socket.NativeAccountNotification.self, from: string.data(using: .utf8)!)
        XCTAssertEqual(result.params?.result?.value.lamports, 41083620)
        XCTAssertThrowsError(try decoder.decode(Socket.TokenAccountNotification.self, from: string.data(using: .utf8)!))
    }
    ...
    func testDecodingTokenAccountNotification() throws {
        ...
        XCTAssertThrowsError(try decoder.decode(Socket.NativeAccountNotification.self, from: string.data(using: .utf8)!))
        
        let result = try decoder.decode(Socket.TokenAccountNotification.self, from: string.data(using: .utf8)!)
    ...
    func testDecodingSignatureNotification() throws {
        ...
        let result = try decoder.decode(Socket.Response<Socket.SignatureNotification>.self, from: string.data(using: .utf8)!)
