...
class APIClientExtensionsTests: XCTestCase {
    ...
    func testCheckAccountValidation() async throws {
        ...
        // TODO:
        // funding SOL address
        let isValid1 = try await apiClient.checkAccountValidation(account: "3h1zGmCwsRJnVk5BuRNMLsPaQu1y2aqXqXDWYCgrp5UG")
        XCTAssertEqual(isValid1, true)
        
        mock.prepare(name: "checkAccountValidation2")
        // no funding SOL address
        let isValid2 = try await apiClient.checkAccountValidation(account: "HnXJX1Bvps8piQwDYEYC6oea9GEkvQvahvRj3c97X9xr")
        XCTAssertEqual(isValid2, false)
        
        mock.prepare(name: "checkAccountValidation3")
        // token address
        let isValid3 = try await apiClient.checkAccountValidation(account: "8J5wZ4Lo7QSwFWwBfWsWUgsbH4Jr44RFsEYj6qFdXYhM")
    ...
    func testCheckIfAssociatedTokenAccountExists() async throws {
        let apiClient = BaseAPIClientMock(endpoint: endpoint)
        let exist = try await apiClient.checkIfAssociatedTokenAccountExists(owner: "9sdwzJWooFrjNGVX6GkkWUG9GyeBnhgJYqh27AsPqwbM", mint: "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v")
        XCTAssertTrue(exist)
        
        let exist2 = try await apiClient.checkIfAssociatedTokenAccountExists(owner: "9sdwzJWooFrjNGVX6GkkWUG9GyeBnhgJYqh27AsPqwbM", mint: "2FPyTwcZLUg1MDrwsyoP4D6s1tM7hAkHYRjkNb5w6Pxk")
    ...
    func testGetTokenWallets() async throws {
        ...
        let res = try await apiClient.getTokenWallets(account: "3h1zGmCwsRJnVk5BuRNMLsPaQu1y2aqXqXDWYCgrp5UG", tokensRepository: tokenRepository)
...
class BaseAPIClientMock: JSONRPCAPIClient {
    ...
    override func getTokenAccountsByOwner(pubkey: String, params: OwnerInfoParams? = nil, configs: RequestConfiguration? = nil) async throws -> [TokenAccount<AccountInfo>] {
    ...
    override func getMultipleAccounts<T: BufferLayout>(pubkeys: [String]) async throws -> [BufferInfo<T>] {
        ...
        let decoder = try JSONDecoder().decode(Rpc<[BufferInfo<Mint>]>.self, from: json.data(using: .utf8)!) as! Rpc<[BufferInfo<T>]>
    ...
    override func getAccountInfo<T: BufferLayout>(account: String) async throws -> BufferInfo<T>? {
        ...
        let decoder = try JSONDecoder().decode(Rpc<BufferInfo<T>?>.self, from: accountInfoResponseJSON.data(using: .utf8)!)
        return decoder.value
    }
