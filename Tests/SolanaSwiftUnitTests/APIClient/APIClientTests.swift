...
class APIClientTests: XCTestCase {
    ...
    func testGetAccountInfoError() async throws {
        ...
        let apiClient = JSONRPCAPIClient(endpoint: endpoint, networkManager: mock)
        do {
            let _: BufferInfo<AccountInfo>? = try await apiClient.getAccountInfo(account: "HWbsF542VSCxdGKcHrXuvJJnpwCEewmzdsG6KTxXMRRk")
...
//        XCTAssert(result[0].pubkey == "57UtuDwoCurTTWySMeV5MiopvDWvK2QeLWu47biQjjLJ")
    ...
    func testGetTokenAccountsByOwner() async throws {
        ...
        let apiClient = JSONRPCAPIClient(endpoint: endpoint, networkManager: mock)
        let result = try await apiClient.getTokenAccountsByOwner(pubkey: "fjohsw9j8aBJaEE5ddzHk3AgMjdbQSXrMoPGrepNHrB", params: .init(mint: nil, programId: TokenProgram.id.base58EncodedString), configs: .init(encoding: "base64"))
...
extension NetworkManagerMock {
    func requestData(request: URLRequest, res: String) async throws -> Data {
