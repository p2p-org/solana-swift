...
class SocketIntegrationTests: XCTestCase {
    ...
    func testSocketEvents() async throws {
        ...
        delegate.onConected = {
            Task {
                let _ = try await self.socket.accountSubscribe(publickey: "9xkso5sXmSEaEME7KDD7EumvYKyYRBJJ1ArQopnCGZrA") // native address
                
                DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1)) {
                    Task {
                        try await self.socket.accountSubscribe(publickey: "2uTfDKywe5ZGqyztJ6KMEPrRTa9WD53ueviYoti8vdCX") // usdc address
                    }
                }
                
                DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(2)) {
                    Task {
                        try await self.socket.signatureSubscribe(signature: "2ctrG8WQWvbawgwY9mr7dv8LXXj4bridpD6b4sEJu7gFgvkrSgd6APu2Xnp6vq4oKEkvoSq9W1QzpVi9gZRrM6WR") // signature status
                    }
                }
