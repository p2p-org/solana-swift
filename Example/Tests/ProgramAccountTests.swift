...
class ProgramAccountTests: XCTestCase {
    ...
    func testDecodingProgramAccountFromBase64() throws {
        ...
        let programAccount = try JSONDecoder().decode(SolanaSDK.ProgramAccount<SolanaSDK.AccountLayout>.self, from: string.data(using: .utf8)!)
        let token = SolanaSDK.Token(layout: programAccount.account.data.value!, pubkey: programAccount.pubkey, in: "devnet")
